...
if uploaded_pdfs:
    for uploaded_pdf in uploaded_pdfs:
        ...
        with st.sidebar.status(f"Indexing {uploaded_pdf.name}…", expanded=True) as status_box:
            summary = ingest_pdf(
            ...
            status_box.update(
                label=f"✅ {uploaded_pdf.name} indexed",
                state="complete",
                expanded=False
            )
        ...
        def ai_only_stream():
            for message_chunk, _ in chatbot.stream(
                ...
                if isinstance(message_chunk, AIMessage):
                    ...
                    # Case 1: Gemini structured output (list of blocks)
                    if isinstance(content, list):
                        for block in content:
                            if isinstance(block, dict):
                                if block.get("type") == "text":
                                    yield block.get("text", "")
                    # Case 2: Normal string output
